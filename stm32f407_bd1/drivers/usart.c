...
int stm32_init(rt_serial_t *serial)
{
    struct stm32_uart* uart;
...
void stm32_deinit(rt_serial_t *serial)
{
...
static void con_init(struct rt_console *con)
{
    uart_init((USART_TypeDef*)con->user_data);
}

static void con_write(struct rt_console *con, char ch)
{
	USART_TypeDef* uart = (USART_TypeDef*)con->user_data;

    while (!(uart->SR & USART_FLAG_TXE));
    uart->DR = ch;
}

static struct rt_console dbgout =
{
    con_init,
	con_write,
	USART3,
