...
#if MAIN_HAS_NOARGC
static MAIN_RETURN_TYPE core_main(void *parameter) {
	int argc=0;
	char *argv[1];
#else
static MAIN_RETURN_TYPE main(int argc, char *argv[]) {
#endif
...
#error "Please define a way to initialize a memory block."
	...
	if (time_in_secs(total_time) > 0)
		...
		//ee_printf("default_num_contexts   : %d\n",default_num_contexts);
#endif
...
#if (MULTITHREAD>1)
			ee_printf(" / %d:%s",MULTITHREAD,PARALLEL_METHOD);
#endif
			ee_printf("\n");
		}
#endif
...
int coremark(int argc, char** argv)
{
	...
	if (argc == 1) iterations = 1600;
	else iterations = atoi(argv[1]);

	seed4_volatile = iterations;

	tid = rt_thread_create("coremark", core_main, RT_NULL, 1024 * 8, 8, 10);
	if (tid != RT_NULL) rt_thread_startup(tid);

	return 0;
}
MSH_CMD_EXPORT(coremark, coremark test);
